...
import 'package:ios_springboard/features/spring_board/components/spring_board_draggable/old_spring_board_drag_avatar.dart';
import 'package:ios_springboard/features/spring_board/screen/portal_root_key.dart';

class OldSpringBoardDraggable extends StatefulWidget {
  const OldSpringBoardDraggable({
    Key? key,
    required this.size,
    required this.child,
  ...
  @override
  State<OldSpringBoardDraggable> createState() =>
      _OldSpringBoardDraggableState();
}

class _OldSpringBoardDraggableState extends State<OldSpringBoardDraggable> {
  ...
  Offset _slotOffset() {
    RenderBox box =
        portalRootKey.currentContext!.findRenderObject() as RenderBox;
  ...
  OldSpringBoardDragAvatar? _startDrag(Offset position) {
    if (_activeCount > 0) {
      return null;
    }
    ...
    final avatar = OldSpringBoardDragAvatar(
  ...
  Offset childDragAnchorStrategy(
    OldSpringBoardDraggable draggable,
